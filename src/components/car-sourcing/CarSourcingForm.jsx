...
export default function CarSourcingForm() {
  ...
  async function handleSubmit(e) {
    ...
    setStatus("sending");
    try {
      await sendFormEmail("Car Sourcing Enquiry", {
  ...
  if (status === "success") {
    return (
      <div className="rounded-xl bg-white shadow-2xl shadow-sky-900/10 border border-gray-100 p-8 flex flex-col items-center justify-center gap-5 text-center min-h-[340px]">
        ...
        <p className="text-sm text-gray-500 font-medium max-w-xs">We'll start sourcing your ideal car and be in touch shortly.</p>
    ...
    <form onSubmit={handleSubmit} className="rounded-xl bg-white shadow-2xl shadow-sky-900/10 border border-gray-100 p-8 space-y-6">
      <div className="grid grid-cols-1 gap-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <input type="text" placeholder="Start typing make"  {...field("make")}  className={inputClass("make")} />
          <input type="text" placeholder="Start typing model" {...field("model")} className={inputClass("model")} />
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <input type="text" placeholder="Year *" {...field("year")} className={inputClass("year")} />
            {errors.year && <p className="mt-1 text-[10px] text-red-500 font-bold">{errors.year}</p>}
          </div>
          <input type="text" placeholder="Variant e.g. TDI" {...field("variant")} className={inputClass("variant")} />
        </div>

        <div className="relative">
          <select {...field("fuelType")} className={inputClass("fuelType") + " appearance-none"}>
        ...
        <textarea rows={4} placeholder="Additional Information *" {...field("additionalInfo")}
